...
public class PrettyPrint
{
    ...
    public void evaluateWidth(PointSet problem)
    { this.column = width(problem); }
    
    public void evaluateWidth(Timer timer)
    { this.column = Math.max(column, width(timer)); }
    
    public void evaluateWidth(TimerSuite timerSuite)
    { this.column = Math.max(column, width(timerSuite)); }
    ...
    private int width(Timer timer)
    {
        int width = 0;
        
        for (PointSet problem : timer.getSortedPointSets())
        { width = Math.max(width, width(problem)); }
        
        return Math.max(width, width(timer.getSolutionName()));
    }
    
    private int width(TimerSuite timerSuite)
    {
        int width = 0;
        
        for (Timer timer : timerSuite.getTimers())
        { width = Math.max(width, width(timer)); }
