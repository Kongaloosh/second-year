...
def run31A_AllFolds():
    ...
    print "Incorrect Classifications: ", incorrectClassification, "totalClassificaitons: ",  totalClassification
...
def run31B_AllFolds():
    ...
    for x in range(1,11): # for all the possible test folds
        ...
        for y in range(0,len(testData)):
            classification = gaussianFullCov(trainingClasses, trainingData, testData[y])            
            actualClass = int(classes[y][0])            
            if classification != actualClass:
...
def gaussianFullCov(trainingLables, trainingData, testData):
    ...
    maxProbability = 0
    mostProbableClass = 0    
    while i<10: # For all the classes (class is -1)
        matrix = [] # generate a matrix
        for j,x in enumerate(trainingLables):
            if (int(x[0])) == i+1: # (class is correct)
        ...
        """   
        probability = 0L + -0.5* numpy.dot(numpy.dot((numpy.transpose(term)), (sigma**-1)),(term))
        
        if probability > maxProbability:
            maxProbability = probability
            mostProbableClass = i+1 # class is correct
        i+=1
...
def multivariateClassification(trainingLables, trainingData, testData):
    ...
    while i<10: # For all the classes
        ...
        sigma =0L+ numpy.cov(matrix,rowvar=0)
        d = len(sigma) 
        mu = numpy.mean(matrix, axis =0)# take the new matrix and form mu and sigma
      ...
      Note: given the posterior probability is assumed to be the same for
            ...
            remove it from the calculation of probabilty. 
           """   
        firstTerm = 0L + -0.5* numpy.dot(numpy.dot((numpy.transpose(term)), (sigma**-1)),(term))
