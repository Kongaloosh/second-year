...
{- --------------- addDomains :: [Var] -> CSP -> Assignment -> Domain  ---------------
	Helper function for numChoices which adds possible choices for 
-}
addDomains :: [Var] -> CSP -> Assignment -> Domain
addDomains [] _ _= []
...
----------+--------+--------+--------+---------+----------+---------+-----------+
ausCsp    | 37     |  11    |   6    |    11   |    11    |  6      |     6     |
mapCsp    | 1176   |  23    |   15   |    23   |    15    |  15     |     15    |
ms3x3Csp  | 3654   |  195   |   110  |    188  |    106   |xxxxxxxxx|xxxxxxxxxxx|
8-Queens  | 876    |  88    |   75   |    47   |    39    |  33     |     21    |
12-Queens | 3066   |  193   |   153  |    209  |    32    |  67     |     22    |
----------+--------+--------+--------+---------+----------+---------+-----------+
	...
	following:
		...
		In particular, compare and explain your results of the following pairs of algorithms:
			...
			1. BT vs. FC
				...
				_some_ errors early on.
				
			2. FC vs. FC+MRV
				The MRV is the application of a heuristic. By assigning values to the most constrained variables
				early on, you avoid having to explore them as branches only to find they have already been constrained.
			...
			6. AC3 & AC3 MRV LCV compared to LCV
		...
		in larger situations the cost of re-computing the consistency will slow the performance of forward-checking.
		This can be remedied by keeping a record of successful and unsuccessful node checks
		
		This has been implemented previously as 'minimal forward checking'
...
{-
 Debug: should trigger false:
	revise (delDomainVal("V",1)$ delDomainVal ("V",2) $ delDomainVal ("NSW",2) $ delDomainVal ("NSW",1) ausCsp) "NSW" "V"
-}
{- --------------- cspRec :: Var -> [Var] -> [Constraint]  ---------------
	Helper funciton for getVars and queenDomains. Given the size of the board,
	the function returns the list of all possible rows a queen could be in--
...
revise' csp x (iX:iXS) y bool
	|existsConsistentValue csp x iX y (getDomain y csp) = revise' csp x iXS y True
	|otherwise = revise' (delDomainVal (x,iX) csp) x iXS y (bool || False) -- this assignment is not consistent, so remove it from the domain. 
